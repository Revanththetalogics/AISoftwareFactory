...
class TestBackupSystemInit:
    ...
    def test_init_with_defaults(self, tmp_path):
        ...
        with (
            patch("backend.infrastructure.backup_system.get_settings") as mock_settings,
    ...
    def test_init_with_custom_values(self, tmp_path):
        ...
        with (
            ...
            backup = BackupSystem(backup_dir=str(tmp_path / "custom_backups"), retention_days=7, schedule="0 0 * * *")
...
class TestCreateBackup:
    ...
    async def test_create_backup_with_name(self, tmp_path):
        ...
        with (
            ...
            with (
                patch.object(backup, "_backup_database", new_callable=AsyncMock) as mock_db,
        ...
        with (
            ...
            for i, name in enumerate(["backup1", "backup2"]):
                ...
                (backup_dir / "metadata.json").write_text(
                    json.dumps({"name": name, "created_at": f"2024-01-0{i + 1}T00:00:00"})
                )
