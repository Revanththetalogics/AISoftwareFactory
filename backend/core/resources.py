...
class ResourceManager:
    ...
    async def _initialize_database(self):
        ...
        try:
            ...
            self._db_engine = create_async_engine(
                ...
                echo=settings.DEBUG,
                connect_args={"server_settings": {"application_name": "theta_ai_backend"}},
            )

            # Create session factory
            self._db_session_factory = async_sessionmaker(
                bind=self._db_engine, class_=AsyncSession, expire_on_commit=False, autoflush=False, autocommit=False
            )
    ...
    async def _initialize_redis(self):
        ...
        try:
            ...
            logger.info("Redis connection pool initialized", extra={"pool_size": settings.REDIS_POOL_SIZE})
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            async with self.get_db_session() as session:
                ...
                health_status["database"] = {"status": "healthy", "latency_ms": latency}
        except Exception as e:
            health_status["database"] = {"status": "unhealthy", "error": str(e), "latency_ms": None}
        ...
        try:
            ...
            latency = round((time.time() - start_time) * 1000, 2)
            health_status["redis"] = {"status": "healthy", "latency_ms": latency}
            await self.close_redis_connection(redis_client)
        except Exception as e:
            health_status["redis"] = {"status": "unhealthy", "error": str(e), "latency_ms": None}
...
class ConnectionMonitor:
    ...
    async def _perform_monitoring(self):
        ...
        if metrics["db_connections_active"] > settings.DB_POOL_SIZE * 0.8:
            logger.warning("High database connection usage", extra={"active": metrics["db_connections_active"]})

        if metrics["redis_connections_active"] > settings.REDIS_POOL_SIZE * 0.8:
            logger.warning("High Redis connection usage", extra={"active": metrics["redis_connections_active"]})

        # Perform health check periodically
        if metrics["db_connection_errors"] > 0 or metrics["redis_connection_errors"] > 0:
            health = await resource_manager.health_check()
            unhealthy_services = [service for service, status in health.items() if status["status"] == "unhealthy"]

            if unhealthy_services:
                logger.error("Unhealthy services detected", extra={"services": unhealthy_services})
