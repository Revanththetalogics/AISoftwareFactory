...
async def database_health_check(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return APIResponse(success=True, data=health_data, message="Database health check completed")
...
async def get_database_statistics(
    ...
    try:
        ...
        cache_stats = None
        if include_cache:
            cache_stats = {"cache_size": len(query_cache.cache), "ttl_seconds": query_cache.ttl}
        ...
        return APIResponse(success=True, data=statistics, message="Database statistics retrieved successfully")
...
async def analyze_database_indexes(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return APIResponse(success=True, data=analysis, message="Index analysis completed")
...
async def get_slow_queries(
    ...
    try:
        ...
        return APIResponse(
            success=True,
            data={"slow_queries": slow_queries, "count": len(slow_queries), "threshold_ms": min_time_ms},
...
async def get_query_optimization_suggestions(table_name: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return APIResponse(success=True, data=table_analysis, message=f"Optimization suggestions for {table_name}")
...
async def analyze_specific_query(
    query_text: str = Query(..., description="SQL query to analyze"), db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        analysis = {"query": query_text, "execution_plan": query_plan, "suggestions": []}
        ...
        return APIResponse(success=True, data=analysis, message="Query analysis completed")
...
async def get_cache_statistics():
    ...
    try:
        cache_stats = {
            ...
            "entries": [
                {"key": key, "age_seconds": time.time() - timestamp}
                for key, (_, timestamp) in query_cache.cache.items()
            ],
        }

        return APIResponse(success=True, data=cache_stats, message="Cache statistics retrieved")
...
async def clear_query_cache():
    ...
    try:
        ...
        return APIResponse(
            success=True, data={"cleared_entries": cache_size}, message=f"Cleared {cache_size} cached entries"
        )
